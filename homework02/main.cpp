...

{
    ...
    this->txttitle.v = titile.split(" ", QString::SkipEmptyParts);   //按空格读取



    if((this->txttitle.v).last() == "\n")

        this->txttitle.v.removeLast();
...

    {
        ...
        getdata.v = str.split(" ", QString::SkipEmptyParts);  //读取成绩数据



        if((getdata.v).last() == "\n")

            getdata.v.removeLast();



        if(getdata.v.size()==0) continue;
...

{
    ...
    for(int i=1; i<this->txttitle.v.size(); i++)
...

{
    ...
    for(int j=0;j<this->txttitle.v.size();j++)

    {

        stream<<"   "<<this->txttitle.v.at(j);
...

{
    ...
    QString datafile = "D:/Homework/try/data.txt";
